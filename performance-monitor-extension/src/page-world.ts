...
/** Same-origin heuristic only; cross-origin is always `external`. Not tracing—just a coarse label. */
function classifyRequestKind(urlStr: string): RequestKind {
  let u: URL
  try {
  ...
  if (
    /\.(js|mjs|cjs|css|map|woff2?|ttf|eot|svg|png|jpe?g|gif|webp|ico|wasm)(\?|$)/i.test(p) ||
    p.includes('/_next/') ||
    p.includes('/static/') ||
    p.includes('/assets/') ||
    p.includes('/chunk') ||
...
function buildRequestPayload(
  ...
  return {
    ...partial,
    durationMs,
    requestKind: classifyRequestKind(partial.url),
  }
}
